...
public class P4_stimulus_auto_hotkey_slave_arrows extends PApplet {
...
int operatingMode = 1;
PFont arial;
int lookHere = PApplet.parseInt(random(1, 5));
int shiftMid = 35;
float circX, circY;
...
public void draw(){
  ...
  if (blinkModeON == true){
    ...
    triangle(width/2, height/11-shiftMid, width/2-width/8, height/3.5f-shiftMid, width/2+width/8, height/3.5f-shiftMid);
    ...
    triangle(width/2, height-height/11+shiftMid, width/2-width/8, height-height/3.5f+shiftMid, width/2+width/8, height-height/3.5f+shiftMid);
    ///////////////////////////////////////// 
    if(frameCount% 11 == 0) {
      fill(200);
      rect(margin, margin, rectSize, rectSize);
...
public void commandCaptions(){
  fill(255);
  textAlign(CENTER);
  switch(operatingMode){
    case 1:
      text("LEFT 20Hz", width/3.5f, height/2);
      text("RIGHT 15Hz", width-width/3.5f, height/2);
    ...
    case 2:
      ...
      break;
    case 3:
      text("AWAY 20Hz", width/3.5f, height/2);
      text("TOWARDS 15Hz", width-width/3.5f, height/2);
      text("UP 12Hz", width/2, height/3);
      text("DOWN 8,57Hz",  width/2, height-height/3);
      break;
  }
...
public void keyReleased() {
  ...
  if (key == 'Q' || key == 'q'){
    blinkModeON = false;
    lookHere = PApplet.parseInt(random(1, 5));
  }
  if (key == 'S' || key == 's'){
    if (blinkModeON ==true){
      operatingMode++;
...
private void prepareExitHandler () {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "P4_stimulus_auto_hotkey_slave_arrows" };
    if (passedArgs != null) {
